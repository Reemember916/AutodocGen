...
# 宏定义：支持列表 / 简单表 / `#define` 行
_MACRO_SECTION_RE = re.compile(
    r"(?ms)^(?:##+\s*宏定义|##+\s*Macros?|###\s*宏)\s*\n(.*?)(?=^##+\s|\Z)"
)
_MACRO_DEFINE_RE = re.compile(
    r"^\s*#\s*define\s+([A-Za-z_]\w*)\s+(.+?)(?:\s*//\s*(.*))?$",
    re.MULTILINE,
)
_MACRO_LIST_RE = re.compile(
    r"^\s*[-*]\s*`?([A-Za-z_]\w*)`?\s*(?:=|：|:)\s*(.+?)(?:\s*[—–-]\s*(.+))?$",
    re.MULTILINE,
)
_MACRO_TABLE_HEADER_RE = re.compile(
    r"\|\s*宏名\s*\|\s*(?:取值|值)\s*\|\s*(?:说明|描述|业务含义)\s*\|"
)
_MACRO_TABLE_ROW_RE = re.compile(
...
class MarkdownExtractor:
    ...
    def parse(self, md_content: str) -> HeaderFileIR:
        ...
        macros = self._parse_macros(md_content)
        return HeaderFileIR(
            file_name=file_name,
    ...
    def _parse_macros(self, md_content: str) -> List[MacroIR]:
        """Parse macro definitions from Markdown (list / table / #define)."""
        if not md_content:
            return []
        found: dict[str, MacroIR] = {}

        def _put(name: str, value: str = "", description: str = "") -> None:
            key = (name or "").strip()
            if not key:
                return
            prev = found.get(key)
            ...
            if prev is None:
                found[key] = MacroIR(name=key, value=val, description=desc)
                return
            if val and not prev.value:
                prev.value = val
            if desc and not prev.description:
                prev.description = desc

        # Prefer dedicated 宏定义 section body when present
        bodies = [md_content]
        sec = _MACRO_SECTION_RE.search(md_content)
        if sec:
            bodies.insert(0, sec.group(1) or "")

        for body in bodies:
            for m in _MACRO_DEFINE_RE.finditer(body):
                _put(m.group(1), m.group(2), m.group(3) or "")
            for m in _MACRO_LIST_RE.finditer(body):
                _put(m.group(1), m.group(2), m.group(3) or "")
            if _MACRO_TABLE_HEADER_RE.search(body):
                for m in _MACRO_TABLE_ROW_RE.finditer(body):
                    name = m.group(1).strip()
                    if name.lower() in {"宏名", "name", "---", ":---"}:
                        continue
                    if re.fullmatch(r"[-:]+", name):
                        continue
                    _put(name, m.group(2), m.group(3))
