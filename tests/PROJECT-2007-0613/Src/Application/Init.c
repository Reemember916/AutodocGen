...
extern Uint32 s_initStateStartTime_u32;
...
static Uint16 CPLDHandshake(void)
{
    ...
    for(l_retry_u16 = 0U; l_retry_u16 < CPLD_HANDSHAKE_PHASE_RETRY_MAX; l_retry_u16++)
    {
        ...
        HARD_XINT_UINT16(CPLD_ADDR_WR_HANDSHAKE_2) = 0xAAAAU;
        /* 给CPLD留出一个最小内部处理时间，再读回结果。 */
        delayUs(CPLD_HANDSHAKE_PHASE_RETRY_DELAY_US);
        l_readData_u16 = HARD_XINT_UINT16(CPLD_ADDR_WR_HANDSHAKE_1);
    ...
    for(l_retry_u16 = 0U; l_retry_u16 < CPLD_HANDSHAKE_PHASE_RETRY_MAX; l_retry_u16++)
    {
        ...
        HARD_XINT_UINT16(CPLD_ADDR_WR_HANDSHAKE_1) = 0x5555U;
        /* 给总线写入和CPLD内部更新留最小稳定时间。 */
        delayUs(CPLD_HANDSHAKE_PHASE_RETRY_DELAY_US);
        l_readData_u16 = HARD_XINT_UINT16(CPLD_ADDR_WR_HANDSHAKE_2);
...
static void CPLDBusHandshakeInit(void)
{
    ...
    /* 获取CPLD上电完成标识 */
    l_temp_1_u16 = HARD_XINT_UINT16(CPLD_ADDR_R_POWERUP_FLAG);
    ...
    if(INVALID == l_powerupReady_u16)
    {
        ...
        HARD_XINT_UINT16(CPLD_ADDR_W_HANDSHAKE_FLAG) = CPLD_DATA_HANDSHAKE_INVALID;
        return;
    }
    ...
    s_initCpldCcdlHeartResult_u16 = INVALID;
    HARD_XINT_UINT16(CPLD_ADDR_W_HANDSHAKE_FLAG) = CPLD_DATA_HANDSHAKE_VALID;
}
 ...
 * 【函数名】:CPLDCcdlHeartCheckUpdate
 *
 * 【功能描述】CPLD CCDL心跳检测更新, 刷新CPLD侧心跳状态
 *
 * 【输入参数说明】NONE
 * 【输出参数说明】NONE
 * 【其他说明】NONE
