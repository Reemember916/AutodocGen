...
CCS_ROOT_DEFAULT="/Applications/ti/ccs1220/ccs/tools/compiler/ti-cgt-c2000_22.6.0.LTS"
CCS_ROOT="${CCS_ROOT:-$CCS_ROOT_DEFAULT}"
CL2000="${CL2000:-$CCS_ROOT/bin/cl2000}"
OUT_DIR="${OUT_DIR:-$ROOT_DIR/build_macos}"
DATE_TAG="$(date +%m%d)"
OUT_NAME="${OUT_NAME:-PROJECT-2007-S01-0001-${DATE_TAG}}"
DATE_OUT_NAME="PROJECT-2007-S01-0001-${DATE_TAG}"

if [[ ! -x "$CL2000" ]]; then
  echo "[ERR] cl2000 not found: $CL2000"
  echo "      set CL2000=/path/to/cl2000 or CCS_ROOT=/path/to/ti-cgt-c2000"
  exit 1
fi
...
INCLUDE_FLAGS=(
  ...
  --include_path="$ROOT_DIR/Include/Application/Control"
...
SOURCES=(
  ...
  "Src/Application/Control/Control_Output.c"
  "Src/Application/Control/Control_Fault.c"
...
if [[ "$OUT_NAME" != "$DATE_OUT_NAME" ]]; then
  cp -f "$OUT_DIR/$OUT_NAME.out" "$OUT_DIR/$DATE_OUT_NAME.out"
  if [[ -f "$OUT_DIR/$OUT_NAME.map" ]]; then
    cp -f "$OUT_DIR/$OUT_NAME.map" "$OUT_DIR/$DATE_OUT_NAME.map"
  fi
  if [[ -f "$OUT_DIR/${OUT_NAME}_linkInfo.xml" ]]; then
    cp -f "$OUT_DIR/${OUT_NAME}_linkInfo.xml" "$OUT_DIR/${DATE_OUT_NAME}_linkInfo.xml"
  fi
  echo "[OK ] Synced dated artifact: $OUT_DIR/$DATE_OUT_NAME.out"
fi
