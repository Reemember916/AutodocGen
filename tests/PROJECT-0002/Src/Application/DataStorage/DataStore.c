...
*       SpiFlashDataRead       ---- 记录数据读取
*       SpiFlashPageProgram    ---- 记录数据写入
*       SpiFlashSectorErase    ---- 记录扇区擦除
*       SpiFlashIsBusy         ---- FLASH忙状态查询
...
Uint32 FlashAddrIsEmpty(Uint32 v_addr_u32,Uint16 v_len_u16)
{
     ...
     && ((v_addr_u32 + v_len_u16) <  (FLASH_BASE_ADDR + (FLASH_SECTOR_LEN * FLASH_SECTOR_LAST) + FLASH_SECTOR_LEN)) )
    {
        ...
        /* 读取一条记录长度数据 */
        (void)SpiFlashDataRead(v_addr_u32, l_buff_u16, l_dataLen_u16);
...
Uint32 DichoFindSectorStartAddr(Uint16 v_sectorNum_u16)
{
    ...
    if( (v_sectorNum_u16 >= FLASH_SECTOR_FIRST) && (v_sectorNum_u16 <= FLASH_SECTOR_LAST))
    {
        ...
        if(ls_shiftAddr_u32 >= (l_startAddr_u32 + FLASH_SECTOR_LEN))
        {
            ...
            /*擦除扇区 */
            SpiFlashSectorErase(ls_shiftAddr_u32);
        }
    }
...
void FlashRecordDataUpdate(void)
{
    ...
    {
        /* 获取故障存储触发标志 */
        l_errStoreFlag_u16 = ControlErrStoreFlagGet();
        ...
        if(FLASH_BUFF_UPDATE_VALID == l_updateFlag_u16)
        {
            ...
            /* 更新当前临时缓存区周期索引数据 */
            StoreDataPack(s_flashRecords_t.tempBuff_t[l_tIndex_u32].dataBuff_u16,DATA_RECORD_NUM);
...
void FlashDataStore(void)
{
    ...
    {
        /* FLASH状态处于不忙时进行数据存储 */
        if(FLASH_NOT_BUSY == SpiFlashIsBusy())
        {
            ...
            if( FLASH_SECTOR_NO_ERASED == s_flashRecords_t.nSectorEraseFlag_u16 )
            {
                ...
                {
                    /* 替换为SPI接口的扇区擦除函数 */
                    SpiFlashSectorErase(l_nextSectorAddr_u32);
                }
            }
            ...
            else if( s_flashRecords_t.storeBuffCount_u32 > s_flashRecords_t.storeCount_u32)
            {
                ...
                /* 将数据写入FLASH */
                SpiFlashPageProgram(s_flashRecords_t.writeAddr_u32, (s_flashRecords_t.tempBuff_t[l_tIndex_u32].dataBuff_u16 + s_flashRecords_t.dataIndex_u16), STORE_WORD_NUM);
...
/* ***************************************************************** */
/**
 *    [函数名]	 FlashRecordDataErase
 *
 *    [功能描述]	 清除周期记录区数据
 *    [输入参数说明] NONE
 *	  [输出参数说明] NONE
 *    [其他说明]	  仅擦除周期记录扇区，不擦除特定数据区和全片FLASH
 *    [返回]		  维护功能执行结果
 */
/* ***************************************************************** */
Uint16 FlashRecordDataErase(void)
{
    Uint16 l_sector_u16 = FLASH_SECTOR_FIRST;
    Uint32 l_addr_u32 = 0UL;
    Uint16 l_waitCnt_u16 = 0U;

    for(l_sector_u16 = FLASH_SECTOR_FIRST; l_sector_u16 <= FLASH_SECTOR_LAST; l_sector_u16++)
    {
        l_addr_u32 = FLASH_BASE_ADDR + ((Uint32)l_sector_u16 * FLASH_SECTOR_LEN);

        if(FLASH_SECTOR_ERASED != FlashSectorIsErased(l_addr_u32))
        {
            SpiFlashSectorErase(l_addr_u32);

            l_waitCnt_u16 = 0U;
            while(l_waitCnt_u16 < SPE_DATA_WRITE_OVER_TIME_MS)
            {
                CycleDogFeed();
                delayUs(332UL);
            ...
            if(l_waitCnt_u16 >= SPE_DATA_WRITE_OVER_TIME_MS)
            {
    ...
    s_flashRecords_t.findStartSectorState_u16 = FIND_STATE_YES;
    s_flashRecords_t.findStartAddrState_u16 = FIND_STATE_YES;
    s_flashRecords_t.writeAddr_u32 = FLASH_BASE_ADDR + ((Uint32)FLASH_SECTOR_FIRST * FLASH_SECTOR_LEN);
    s_flashRecords_t.startSector_u16 = FLASH_SECTOR_FIRST;
    s_flashRecords_t.nSectorEraseFlag_u16 = FLASH_SECTOR_ERASED;
    (void)SpeDataWrite(SPE_DATA_DINDEX_FLASH_STORE_SECTOR, FLASH_SECTOR_FIRST);
