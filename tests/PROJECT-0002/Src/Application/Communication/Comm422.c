...
static Uint32 CommMaintAddrConcatU32(Uint8 *vp_buff_u8, Uint16 v_baseIdx_u16, Uint16 v_byteNum_u16);
static Uint16 CommMaintParamChecksumGet(Uint16 v_data_u16);

/* ***************************************************************** */
/**
 * [函数名]	Comm422RxBufferClear
 * [功能描述]	清空指定RS422通道的接收缓存(索引清零 + 全缓存清零)
 * [输入参数说明]v_commID_u16 ---- 通道ID
 * [输出参数说明]NONE
 * [其他说明]	本文件422接收缓存清零的统一入口，供
 *             Comm422RxBufferCompact / Comm422FrameCleanup / Comm422Init
 *             及 Comm422DataBuffRead 复用，避免重复的清零循环样板
 * [返回]	NONE
 */
/* ***************************************************************** */
static void Comm422RxBufferClear(Uint16 v_commID_u16)
...
/* ***************************************************************** */
/**
 * [函数名]	CommMaintAddrConcatU32
 * [功能描述]	将维护报文中多个字节按从高到低(MSB在前)拼接为32位地址
 * [输入参数说明]vp_buff_u8   ---- 数据缓冲区指针
 *             v_baseIdx_u16 ---- 最高字节所在索引(MSB)
 *             v_byteNum_u16 ---- 待拼接字节数(1~4)
 * [输出参数说明]NONE
 * [其他说明]	本文件维护报文起始地址/下载地址/长度等3处多字节拼接的
 *             统一入口，拼接顺序为 base, base-1, ..., base-(byteNum-1)，
 *             每次左移8位后累加低8位，与各调用点原语义完全一致
 * [返回]	拼接得到的32位地址
 */
/* ***************************************************************** */
static Uint32 CommMaintAddrConcatU32(Uint8 *vp_buff_u8, Uint16 v_baseIdx_u16, Uint16 v_byteNum_u16)
{
    Uint32 l_addr_u32 = 0UL;       /* 拼接结果地址 */
    Uint16 l_ii_u16 = 0U;          /* 循环索引 */

    /* 从最高字节开始，逐字节左移拼接 */
    for(l_ii_u16 = 0U; l_ii_u16 < v_byteNum_u16; l_ii_u16++)
    {
        l_addr_u32 = l_addr_u32 << 8U;
        l_addr_u32 = l_addr_u32 + (Uint32)(vp_buff_u8[v_baseIdx_u16 - l_ii_u16] & 0xFFU);
    }
...
/* ***************************************************************** */
/**
 * 【函数名】:CommMaintParamChecksumGet
 *
 * 【功能描述】维护参数校验计算
 * 【输入参数说明】v_data_u16 ---- 16位维护参数
 * 【输出参数说明】NONE
 * 【其他说明】       与特定数据存储的单字节校验口径一致：低/高字节求和后取反
 * 【返回】           低8位校验值
 */
/* ***************************************************************** */
static Uint16 CommMaintParamChecksumGet(Uint16 v_data_u16)
{
    ...
    l_sum_u16 = (v_data_u16 & 0xFFU) + ((v_data_u16 >> 8U) & 0xFFU);
    return ((~l_sum_u16) & 0xFFU);
}
...
static void MaintCommInfoProcess(Uint16 v_commID_u16,Uint16 v_sIndex_u16)
{
    ...
    if( (v_commID_u16 < COMM422_ID_NUM) && (v_sIndex_u16 < COMM_MAINT_RX_FRAME_LEN))
    {
        ...
        /*******************************************/
        /* 对读取起始地址3个字节数据进行拼接(MSB在+8索引,低到高字节) */
        l_Addr_u32 = CommMaintAddrConcatU32((Uint8 *)s_rs422CommBuff_t[v_commID_u16].commBuff_u16,
                                             (Uint16)(v_sIndex_u16 + 8U),
                                             (Uint16)3U);
...
/* ***************************************************************** */
/**
 * [函数名]	Comm422CommInfoGet
 * [功能描述]	获取指定RS422通道当前通信状态快照
 * [输入参数说明]v_commID_u16 ---- 通道ID
 * [输出参数说明]NONE
 * [其他说明]	通道越界时返回清零后的默认结构体
 * [返回]	Rs422CommInfo_t
 */
/* ***************************************************************** */
...
void MaintFuncDataProcess(Uint16 v_commID_u16,Uint16 v_sIndex_u16)
{
    ...
    Uint16 l_hardVersion_u16 = 0U; /* 硬件版本调整值 */
    Uint16 l_hardVersionChecksum_u16 = 0U; /* 硬件版本调整校验 */
    Uint16 l_hardVersionParaChanged_u16 = INVALID; /* 硬件版本参数是否变化 */
    ...
    if( (v_commID_u16 < COMM422_ID_NUM) && (v_sIndex_u16 < COMM_MAINT_RX_FRAME_LEN))
    {
        ...
        {
            /* 对下载起始地址4个字节数据进行拼接(MSB在+8索引,低到高字节) */
            l_startAddr_u32 = CommMaintAddrConcatU32((Uint8 *)s_rs422CommBuff_t[v_commID_u16].commBuff_u16,
                                                     (Uint16)(v_sIndex_u16 + 8U),
                                                     (Uint16)4U);

            /*******************************************/
            /* 对下载地址长度4个字节数据进行拼接(MSB在+12索引,低到高字节) */
            l_addrLen_u32 = CommMaintAddrConcatU32((Uint8 *)s_rs422CommBuff_t[v_commID_u16].commBuff_u16,
                                                   (Uint16)(v_sIndex_u16 + 12U),
                                                   (Uint16)4U);
        ...
        else if(GROUND_MAINT_FUNC_HW_VERSION_ADJUST == l_MaintFunc_u16)
        {
            l_hardVersion_u16 =
                (s_rs422CommBuff_t[v_commID_u16].commBuff_u16[v_sIndex_u16 + 5U] & 0xFFU) |
                ((s_rs422CommBuff_t[v_commID_u16].commBuff_u16[v_sIndex_u16 + 6U] & 0xFFU) << 8U);
            l_hardVersionChecksum_u16 = s_rs422CommBuff_t[v_commID_u16].commBuff_u16[v_sIndex_u16 + 7U] & 0xFFU;

            if((l_hardVersion_u16 != s_rsMaintData_t.hardVersion_u16) ||
               (l_hardVersionChecksum_u16 != s_rsMaintData_t.hardVersionChecksum_u16))
            {
                l_hardVersionParaChanged_u16 = VALID;
            }
        }

        /* 功能变化或下载参数变化时刷新执行触发 */
        if((VALID == l_funcChanged_u16) ||
           (VALID == l_downloadParaChanged_u16) ||
           (VALID == l_hardVersionParaChanged_u16))
        {
            /* 更新维护功能指令  */
            ...
            if(GROUND_MAINT_FUNC_DATA_DOWNLOAD == s_rsMaintData_t.MaintFuncCode_u16)
            {
                ...
                s_rsMaintData_t.hardVersion_u16 = 0U;
                s_rsMaintData_t.hardVersionChecksum_u16 = 0U;
            }
            else if(GROUND_MAINT_FUNC_HW_VERSION_ADJUST == s_rsMaintData_t.MaintFuncCode_u16)
            {
                s_rsMaintData_t.downLoadStartAddr_u32 = 0UL;
                s_rsMaintData_t.downLoadAddrLen_u32 = 0UL;
                s_rsMaintData_t.hardVersion_u16 = l_hardVersion_u16;
                s_rsMaintData_t.hardVersionChecksum_u16 = l_hardVersionChecksum_u16;
            }
            else
            {
                s_rsMaintData_t.downLoadStartAddr_u32 = 0UL;
...
void DownLoadDataCommSend(Uint32 v_startAddr_u32,Uint32 v_addrLen_u32)
{
    ...
    /* 维护下载只允许访问周期记录区，输入地址按记录区内相对地址解释。 */
    if(v_startAddr_u32 >= l_flashWinLen_u32)
    {
        return;
    }

    l_startAddr_u32 = v_startAddr_u32 + (FLASH_BASE_ADDR + (FLASH_SECTOR_FIRST * FLASH_SECTOR_LEN));

    if(v_addrLen_u32 > (l_flashWinLen_u32 - v_startAddr_u32))
    {
        return;
    }
    else
    ...
    while(l_currAddr_u32 < l_endAddr_u32)
    {
        ...
        /* 从内存中读取数据 */
        SpiFlashDataRead(l_currAddr_u32, l_buff_u16, l_sendLen_u16);
 ...
 * 【输入参数说明】NONE
 * 【输出参数说明】NONE
 * 【其他说明】	下载窗口限制在周期记录区内，越界请求不再折返取模
 * 【返回】               维护功能执行结果
 */
/* ***************************************************************** */
Uint16 GroundMaintProcessDataDownLoad(void)
{
    const RsMaintDataInfo_t * l_pMaintData_t = CommMaintDataGet();
    Uint32 l_flashWinLen_u32 = ((FLASH_SECTOR_LAST - FLASH_SECTOR_FIRST) + 1UL) * FLASH_SECTOR_LEN;

    if(NULL == l_pMaintData_t)
    {
        return MAINT_FUNC_EXE_RESULT_INVALID_PARA;
    }

    if((0UL == l_pMaintData_t->downLoadAddrLen_u32) ||
       (l_pMaintData_t->downLoadStartAddr_u32 >= l_flashWinLen_u32) ||
       (l_pMaintData_t->downLoadAddrLen_u32 > (l_flashWinLen_u32 - l_pMaintData_t->downLoadStartAddr_u32)))
    {
        return MAINT_FUNC_EXE_RESULT_INVALID_PARA;
    }

    DownLoadDataCommSend(l_pMaintData_t->downLoadStartAddr_u32,l_pMaintData_t->downLoadAddrLen_u32);
    return MAINT_FUNC_EXE_RESULT_OK;
}
 ...
 * 【输入参数说明】NONE
 * 【输出参数说明】NONE
 * 【其他说明】       计算结果通过维护发送ID5周期上报
 * 【返回】           维护功能执行结果
 */
/* ***************************************************************** */
Uint16 GroundMaintProcessSoftwCRC(void)
{
    /* 周期喂狗 */
    CycleDogFeed();
 ...
 *
 * 【功能描述】地面维护信息擦除功能处理
 * 		   仅擦除周期记录区，避免维护清记录时破坏特定数据区。
 * 【输入参数说明】NONE
 * 【输出参数说明】NONE
 * 【其他说明】        记录区重新初始化后从首扇区开始存储
 * 【返回】            维护功能执行结果
 */
/* ***************************************************************** */
Uint16 GroundMaintProcessDataErase(void)
{
    /* 周期喂狗  */
    CycleDogFeed();

    return FlashRecordDataErase();
}

/* ***************************************************************** */
/**
 * 【函数名】:GroundMaintProcessHardVersionAdjust
 *
 * 【功能描述】地面维护硬件版本调整处理
 * 【输入参数说明】NONE
 * 【输出参数说明】NONE
 * 【其他说明】       使用特定数据区硬件版本存储项，最多尝试3次写入并回读确认
 * 【返回】           维护功能执行结果
 */
/* ***************************************************************** */
Uint16 GroundMaintProcessHardVersionAdjust(void)
{
    const RsMaintDataInfo_t *l_pMaintData_t = CommMaintDataGet();
    Uint16 l_ii_u16 = 0U;
    Uint16 l_writeState_u16 = SPE_DATA_STATE_ERR;
    ...
    if((0U == l_pMaintData_t->hardVersion_u16) ||
       (CommMaintParamChecksumGet(l_pMaintData_t->hardVersion_u16) !=
        (l_pMaintData_t->hardVersionChecksum_u16 & 0xFFU)))
    {
        return MAINT_FUNC_EXE_RESULT_INVALID_PARA;
    }
    ...
    {
        l_writeState_u16 = SpeDataWrite(SPE_DATA_DINDEX_HARDW_VER, l_pMaintData_t->hardVersion_u16);
        if((SPE_DATA_STATE_OK == l_writeState_u16) &&
           (l_pMaintData_t->hardVersion_u16 == SpeDataRead(SPE_DATA_DINDEX_HARDW_VER)))
        {
            return MAINT_FUNC_EXE_RESULT_OK;
        }
    }

    return MAINT_FUNC_EXE_RESULT_FAIL;
}

/* ***************************************************************** */
/**
 * 【函数名】:GroundMaintProcessBitClear
 *
 * 【功能描述】地面维护BIT信息清除处理
 * 【输入参数说明】NONE
 * 【输出参数说明】NONE
 * 【其他说明】       清除IFBIT/MBIT的当前结果和计数；PuBIT为上电历史结果，不在运行期维护命令中清除
 * 【返回】           维护功能执行结果
 */
/* ***************************************************************** */
Uint16 GroundMaintProcessBitClear(void)
{
    CycleDogFeed();
...
/* ***************************************************************** */
/**
 * 【函数名】:CommMaintSoftwCrcGet
 *
 * 【功能描述】获取维护口软件CRC
 * 【输入参数说明】v_crcID_u16 ---- CRC类型索引
 * 【输出参数说明】NONE
 * 【其他说明】       0为应用软件CRC，1为加载软件CRC
 * 【返回】           CRC值
 */
/* ***************************************************************** */
Uint16 CommMaintSoftwCrcGet(Uint16 v_crcID_u16)
{
    Uint16 l_crc_u16 = s_softwCRC_u16;

    if(SOFTW_V_UPODATE_ID == v_crcID_u16)
    {
        l_crc_u16 = s_softwCRC_Update_u16;
    }
...
void Comm422Init(void)
{
    ...
    {
        /* 422通信接收缓冲区初始化: 索引清零 + 全缓存清零(按各通道配置长度) */
        Comm422RxBufferClear(l_ii_u16);
    ...
    s_rsMaintData_t.hardVersion_u16       = 0U;   /* 硬件版本调整值清零 */
    s_rsMaintData_t.hardVersionChecksum_u16 = 0U; /* 硬件版本调整校验清零 */
