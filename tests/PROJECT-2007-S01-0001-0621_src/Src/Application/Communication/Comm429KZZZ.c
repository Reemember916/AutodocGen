 ...
 *    【功能描述】:  KZZZ接收字SSM有效性判定：全标签统一校验SSM=SSM_NORM
 *    【输入参数说明】v_data_un ---- 接收的429字联合
 *    【输出参数说明】NONE
 *    【其他说明】   与RIU一致，所有接收标签均要求SSM_NORM，否则判无效
 *    【返回】       VALID/INVALID
 */
/* ***************************************************************** */
...
/* ***************************************************************** */
/* 函数名: Comm429KZZZSendPreFuel
 * 功能描述: 控制装置429预选油量发送
 * 输入参数说明: v_kzzzID_u16 --- 吊舱通道号 v_data_f --- 预选油量
 * 输出参数说明: NONE
 * 其他说明:    ICD定义分辨率100kg数据位从bit19开始
 * 返回:        NONE */
/* ***************************************************************** */
...
/* ========================================================================== */
/* ***************************************************************** */
/* KZZZ周期发送（含事件量边沿检测与对侧表驱动） */
/* ***************************************************************** */

/* 发送缓存类型定义 */
typedef struct
{
    Uint16 periodicDiv_u16;
    Uint16 lastMbitCmd_u16;
    Uint16 lastPzValid_u16;
    Uint16 lastLpPreFuel_u16;
    Uint16 lastRpPreFuel_u16;
} KZZZTxCache_t;

static KZZZTxCache_t s_kzzzTxCache_t =
{
    0U, 0xFFFFU, 0xFFFFU, 0xFFFFU, 0xFFFFU
};

#define KZZZ_PERIODIC_SEND_DIV        (4U)
#define KZZZ_BIT_EVENT_NUM            (2U)

static const Uint16 s_bitEventRiuIdx_u16[KZZZ_BIT_EVENT_NUM] =
{
    RIU_R_DATA_INDEX_LIFE_INFO,
    RIU_R_DATA_INDEX_OIL_RESET,
};

static const Uint16 s_sideToKzzz_u16[COMM429_KZZZ_NUM] =
{
    COMM429_KZZZ_1,
    COMM429_KZZZ_2,
};

static Uint16 s_bitEventLastVal_u16[KZZZ_BIT_EVENT_NUM][COMM429_KZZZ_NUM] =
{
    {0xFFFFU, 0xFFFFU},
    {0xFFFFU, 0xFFFFU},
};
...
/* ***************************************************************** */
/**
 *【函数名】: KZZZTxCacheReset
 *【功能描述】复位KZZZ周期发送缓存
 *【输入参数说明】NONE
 *【输出参数说明】NONE
 *【其他说明】:       清空分频器和边沿检测缓存，避免沿用上一轮发送状态
 *【返回】:              NONE
 ***************************************************************** */
static void KZZZTxCacheReset(void)
{
    ...
    s_bitEventLastVal_u16[0U][0U] = 0xFFFFU;
    s_bitEventLastVal_u16[0U][1U] = 0xFFFFU;
    s_bitEventLastVal_u16[1U][0U] = 0xFFFFU;
    s_bitEventLastVal_u16[1U][1U] = 0xFFFFU;
}
 ...
 *【输入参数说明】vp_orig_t   --- 原始429字指针
 *                v_shift_u16--- data域内右移位数
 *【输出参数说明】NONE
 *【其他说明】:       空指针时按0处理，便于上层统一走默认发送路径
 *【返回】:              bit值
 ***************************************************************** */
static Uint16 RIUOrigBitGet(const Orig429Data_t *vp_orig_t, Uint16 v_shift_u16)
{
    union arinc429Data l_rdata_un;
...
/**
 *【函数名】: RIUOrigBitsGet
 *【功能描述】从RIU原始429字中提取多bit字段
 *【输入参数说明】vp_orig_t   --- 原始429字指针
 *                v_shift_u16--- data域内右移位数
 *                v_mask_u16 --- 取值掩码
 *【输出参数说明】NONE
 *【其他说明】:       与RIUOrigBitGet保持同样的空指针兜底口径
 *【返回】:              字段值
 ***************************************************************** */
static Uint16 RIUOrigBitsGet(const Orig429Data_t *vp_orig_t, Uint16 v_shift_u16, Uint16 v_mask_u16)
{
    union arinc429Data l_rdata_un;
    l_rdata_un.msgData = 0UL;
    if(NULL != vp_orig_t)
 ...
 *【功能描述】按控制链口径向左右KZZZ发送周期信息和事件量
 *【输入参数说明】NONE
 *【输出参数说明】NONE
 *【其他说明】:       同时处理时间帧、周期慢速帧和边沿触发事件帧
 *【返回】:              NONE
 ***************************************************************** */
