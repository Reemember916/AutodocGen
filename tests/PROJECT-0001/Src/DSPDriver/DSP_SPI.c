 ...
 * 【函数名】:SpiGpioConf
 *
 * 【功能描述】SPI GPIO 引脚复用配置
 *
 * 【输入参数说明】l_SimoGpio_u8  ---- SIMO 引脚号
 *              l_SomiGpio_u8  ---- SOMI 引脚号
 *              l_ClkGpio_u8   ---- CLK 引脚号
 *              l_SpisteGpio_u8 ---- SPISTE 引脚号
 * 【输出参数说明】NONE
 * 【其他说明】    根据传入的引脚号分别配置 GPAMUX/GPBMUX 寄存器
 * 【返回】        NONE
 */
/* ***************************************************************** */
...
void SpiWriteBuff(Uint16 *l_buff_u16, Uint16 l_len_u16)
{
    ...
    for( l_ii_u16 = 0U; l_ii_u16 < l_len_u16; )
    {
        /* 等待发送缓冲区就绪 */
        if( SPI_TX_READY == SpiIsTxReady() )
        {
            SpiWrite(l_buff_u16[l_ii_u16]);
            l_ii_u16++;
...
void SpiReadBuff(Uint16 *l_buff_u16, Uint16 l_len_u16)
{
    ...
    if(NULL != l_buff_u16)
    {
        for( l_ii_u16 = 0U; l_ii_u16 < l_len_u16; l_ii_u16++)
        {
            if( SPI_RX_READY == SpiIsRxReady() )
            {
                l_buff_u16[l_ii_u16] = SpiaRegs.SPIRXBUF;
            }
            else
            {
                l_buff_u16[l_ii_u16] = 0xFFU;
                s_SpiErrCount_u16++;
            }
        }
    }
